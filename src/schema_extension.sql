...
BEGIN
    INSERT INTO BUS (BusCompany, Registration, RegistrationYear, BusStatus, Capacity, TyreHealth, EngineHealth, ChassisHealth)
...
-- Seed active trips for simulation (only if TRIP is empty AND buses exist)
IF NOT EXISTS (SELECT 1 FROM TRIP) AND EXISTS (SELECT 1 FROM BUS WHERE BusId = 1)
BEGIN
    DECLARE @BusId1 INT = (SELECT TOP 1 BusId FROM BUS ORDER BY BusId);
    DECLARE @BusId2 INT = (SELECT TOP 1 BusId FROM BUS ORDER BY BusId DESC);
    DECLARE @RouteId1 INT = (SELECT TOP 1 RouteId FROM ROUTEE ORDER BY RouteId);
    DECLARE @RouteId2 INT = (SELECT TOP 1 RouteId FROM ROUTEE ORDER BY RouteId DESC);
    DECLARE @DriverId INT = (SELECT TOP 1 UserId FROM DRIVER ORDER BY UserId);

    IF @BusId1 IS NOT NULL AND @RouteId1 IS NOT NULL AND @DriverId IS NOT NULL
    BEGIN
        INSERT INTO TRIP (RouteId, BusId, DriverId, DepartureTime, ArrivalTime,
                          CurrentStopIndex, SimulatedProgress, TripStatus)
            VALUES (@RouteId1, @BusId1, @DriverId,
                    GETDATE(), DATEADD(HOUR, 1, GETDATE()),
                    0, 0.0, 'IN_PROGRESS');

        IF @BusId2 <> @BusId1 AND @RouteId2 <> @RouteId1
        BEGIN
            INSERT INTO TRIP (RouteId, BusId, DriverId, DepartureTime, ArrivalTime,
                              CurrentStopIndex, SimulatedProgress, TripStatus)
                VALUES (@RouteId2, @BusId2, @DriverId,
                        GETDATE(), DATEADD(HOUR, 1, GETDATE()),
                        0, 0.0, 'IN_PROGRESS');
        END
    END
